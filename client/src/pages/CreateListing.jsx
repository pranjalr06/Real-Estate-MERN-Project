        ...
        <form onSubmit={handleSubmit} className='flex flex-col sm:flex-row gap-4'>
          ...
          <div className=' flex flex-col gap-4 flex-1'>
            ...
            <div className='flex gap-6 flex-wrap'>
              <div className='flex gap-2'>
                <input type="checkbox" id='Sale' className='w-5 border cursor-pointer  border-black' onChange={handleChange} checked={formData.type === 'Sale'} />
                <span>Sale</span>
              </div>
              ...
              <div className='flex gap-2'>
                <input type="checkbox" id='parking' className='w-5 cursor-pointer ' onChange={handleChange} checked={formData.parking} />
                <span>Parking Spot</span>
              </div>

              <div className='flex gap-2'>
                <input type="checkbox" id='furnished' className='w-5 cursor-pointer ' onChange={handleChange} checked={formData.furnished} />
                <span>Furnished</span>
              </div>

              <div className='flex gap-2'>
                <input type="checkbox" id='offer' className='w-5 cursor-pointer ' onChange={handleChange} checked={formData.offer} />
                <span>Offer</span>
              </div>
            </div>
            ...
            <div className='flex flex-wrap gap-6'>
              <div className='flex items-center gap-2 my-2'>
                <input type="number" id='bedrooms' min='1' max='10' required
                  className='p-3 border border-neutral-400 rounded-lg' onChange={handleChange} value={formData.bedrooms} />
                <p>Beds</p>
              </div>

              <div className='flex items-center gap-2 my-2'>
                <input type="number" id='bathrooms' min='1' max='10' required
                  className='p-3 border border-neutral-400 rounded-lg' onChange={handleChange} value={formData.bathrooms} />
                <p>Bathrooms</p>
              </div>

              <div className='flex items-center gap-2 my-2'>
                <input type="number" id='regularPrice' min='2000' max='1000000' required
                  className='p-3 border border-neutral-400 rounded-lg' onChange={handleChange} value={formData.regularPrice} />
              ...
              {/* // for offer functionality */}
              {formData.offer && (
                <div className='flex items-center gap-2 my-2'>
                  <input type="number" id='discountPrice' min='0' max='1000000' required
                    className='p-3 border border-neutral-400 rounded-lg' onChange={handleChange} value={formData.discountPrice} />
                  <div className="flex flex-col items-center">
          ...
          <div className="flex flex-col flex-1 gap-4">
            ...
            <div className="flex gap-4 ">
              <input onChange={(e) => setFiles(e.target.files)} className='p-3 border border-neutral-400 rounded w-full  cursor-pointer ' type="file" id='images' accept='image/*' multiple />
              <button className='p-3 cursor-pointer  text-green-700 border border-green-700 rounded uppercase hover:shadow-lg disabled:opacity-80'
            ...
            <button disabled={loading || uploading} className='p-2 bg-slate-700 text-white rounded-lg uppercase hover:opacity-95 disabled:opacity-80'>{loading ? 'Creating...' : 'Create Listing'}</button>
